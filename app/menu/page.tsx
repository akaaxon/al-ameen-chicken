...
function MenuGrid() {
  ...
  // Track previous category so we know when to trigger the entrance animation
  ...
  // --- SMART RESET LOGIC (FIXED ANIMATION & SKELETON FLASH) ---
  useEffect(() => {
    setOffset(0);
    setHasMore(true);

    // Only clear screen and re-animate if the CATEGORY changed
    if (prevCategoryRef.current !== activeCategory) {
      setProducts([]);
      setLoading(true);
      hasAnimatedRef.current = false; // Trigger GSAP
      prevCategoryRef.current = activeCategory;
    }
    // If it's just a search change, we don't clear the products or reset animation.
    // The fetch request below will simply overwrite the grid instantly.
    ...
    async function fetchPage() {
      try {
        ...
        if (debouncedSearchQuery) {
          ...
          // Overwrite items directly without triggering loading skeleton
          setProducts(data);
          setHasMore(false); 
